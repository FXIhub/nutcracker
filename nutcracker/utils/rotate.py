...
def rotate_based_on_quaternion(input_model,quat,quat_is_extrinsic=False):
    """
    Rotate a given model based on a given quaternion by calculating the euler angles. Standart x convention is used, that means a rotation around z-x'-z''.
...
def find_rotation_between_two_models(model_1,model_2,full_output=False,model_1_is_intensity=True,model_2_is_intensity=True,shift_range=3):
    """
    Finding the right alignment by rotating/shifting one model in accordance with standart x convention and using a brute force algorithm to minimise the difference between the two models.
